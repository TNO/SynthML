...
For every core activity $\coreActivity$, the language of $\coreActivity$, denoted $L(\coreActivity)$, is the set of all legal sequences $\omega \in T^*$ of edges firings starting from the initial configuration of tokens $c_0$. 
...
\begin{proof}
We must show that $L(\coreActivity) - \{ e_1, e_2 \} \subseteq L(\coreActivity') - \{ e_F \}$ and that $L(\coreActivity) -\{ e_1, e_2 \} \supseteq L(\coreActivity') - \{ e_F \}$. 
