...
Data-based synthesis computes a minimally restrictive supervisor for going from a state that satisfies the activity precondition, to a state that satisfies the postcondition, without violating requirements, running into deadlocks, etc.
...
The state space that is generated from the synthesized supervisor expresses all possible orderings of events, taking into account the extra synthesized guards and the original action guards as specified in the UML model.
...
Noteworthy is that this state space will have state annotations $\texttt{@state(\dots)}$ that indicate the values of all variables in every location.
...
The single CIF final location that is guaranteed by \ensureSingleSourceAndSinkLabel{} will directly correspond to the single final node in the to-be-synthesized activity.
...
Notably, the input should be a CIF specification containing exactly one deterministic automaton with a exactly one initial location.
...
Notably, the result is a CIF specification containing a single minimal DFA that is language equivalent to the DFA in the input CIF specification.
...
Our main strategy is therefore to synthesize a minimal free-choice Petri Net from \var{cifStatespace_{min}} (in this section), and translate that to an UML activity (in \autoref{sec:transform-petrinet-to-activity}).
We thus use Petri Nets as an intermediate formalism in our activity synthesis algorithm.

However, translating a free-choice Petri Net to an UML activity structure is not yet sufficient: we then still need to compute the control flow guards for the decision nodes of the synthesized UML activity.
...
This is because Petrify seems unable to do Petri Net synthesis in case the input automaton has deadlock locations.
...
It has no incoming arcs and exactly one outgoing arc to a transition named \startLabel{}.
...
This is done by Petrify for technical reasons that are not explained further here, in order to enable Petri Net synthesis.
...
Since activities are reasonably close to Petri Nets, we do this by first synthesizing a free-choice Petri Net using known techniques, and transforming this Petri Net to a UML activity using \transformToActivityLabel{}.
...
Furthermore, \transformToActivityLabel{} ensures that all control flows that go out of a decision node will end in some (call behavior) action node.
...
Given any control flow in the UML activity that goes from some decision node \var{D} to some (call behavior) action node \var{A}, the guard for this control flow can be computed as $\texttt{simplify}(\texttt{simplify}(\var{A_{cond}}, \var{A_{guard}}), \var{D_{pred}})$, where:
\begin{itemize}
	\item \var{A_{guard}} is the original guard predicate of $A$ as defined in the input UML model.
	\item \var{A_{cond}} is the extra synthesized condition predicate for $A$, that was computed by data-based synthesis.
...
We can further simplify against \var{D_{pred}}, i.e, the knowledge of which states the system may be in while evaluating $D$, as it does not make sense for the control flow guard to restrict irrelevant system states.
