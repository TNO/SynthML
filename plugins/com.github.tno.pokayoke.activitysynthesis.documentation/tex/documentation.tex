...
% Extra commands for common constructs in the document body.
...
\begin{algorithm}[t!]
	...
	\procedure{\synth{\var{umlModel}}}{
		...
		\var{cifSpec} := \keyw{transform-uml-to-cif}(\var{umlModel})\;
		\var{cifSupervisor} := \keyw{data-based-synthesis}(\var{cifSpec})\;
		...
		\var{cifStatespace_{proj}} := \keyw{event-based-projection}(\var{cifStatespace})\;
		...
		\var{cifStatespace_{red}} := \keyw{reduce-state-annotations}(\var{cifStatespace})\;
		\var{umlActivity_{guards}} := \keyw{compute-edge-guards}($\var{cifStatespace_{red}}, \dots$)\;
...
For every operation in \autoref{alg:activity-synthesis} a short explanation is given, followed by the motivation for having that operation, followed by its preconditions and postconditions.
...
The operation $\keyw{data-based-synthesis}(\var{cifSpec})$ executes the data-based supervisory controller synthesis tool that comes with ESCET/CIF.
...
Once we have the UML activity structure, we need to compute the guards of the outgoing edges of its decision nodes (see \autoref{sec:compute-edge-guards}).
...
The operation $\keyw{generate-statespace}(\var{cifSupervisor})$ executes the CIF Explorer tool that comes with Eclipse ESCET\footnote{See \url{https://eclipse.dev/escet/cif/tools/explorer.html}.}.
...
\begin{itemize}
	...
	That is, any path from any location in the state space will either end up in a marked location, or will loop forever.
	In other words, the only locations from which no further edges can be taken are the marked locations.
	\item Unless the supervisor was empty (in which case the synthesis chain will have crashed already before generating the state space), there is at least one initial location and at least one marked location.
...
The operation $\keyw{ensure-single-source-and-sink}(\var{cifStatespace})$ transforms the single automaton in the given CIF specification, $\var{cifStatespace}$, to ensure it has exactly one initial (source) location and exactly one marked (sink) location.
...
Thus, the single CIF initial location that is guaranteed by $\keyw{ensure-single-source-and-sink}$ will then directly correspond to the single initial node in the synthesized activity.
...
So $\keyw{ensure-single-source-and-sink}$ ensures that the CIF specification already has the right structure with respect to that.
...
The operation $\keyw{event-based-projection}(\var{cifStatespace})$ projects the single automaton in the given CIF specification for all controllable events.
...
The goal of Petri Net synthesis is to find a minimal Petri Net whose behavior is trace equivalent to the CIF state space that was given as input to Petri Net synthesis.
...
\paragraph{Preconditions.}
Since $\keyw{event-based-projection}$ uses the automaton projection tool that comes with Eclipse ESCET\footnote{See \url{https://eclipse.dev/escet/cif/tools/eventbased/projection.html}.}, all preconditions from that tool apply.
...
The $\keyw{dfa-minimization}(\var{cifStatespace_{proj}})$ operation minimizes the single deterministic automaton in the given CIF specification, \var{cifStatespace_{proj}}.
...
\paragraph{Preconditions.}
Since $\keyw{dfa-minimization}$ uses the Event-based DFA minimization tool that comes with Eclipse ESCET\footnote{See \url{https://eclipse.dev/escet/cif/tools/eventbased/dfa-minimize.html}.}, all preconditions from that tool apply.
...
Note that the previous steps of the activity synthesis algorithm ensure that the input we give to $\keyw{dfa-minimization}$ is deterministic and has exactly one initial location.
...
(Minimal DFAs are also unique in the sense that there cannot exist two different minimal DFAs with the same language.
...
The operation $\keyw{petrinet-synthesis}(\var{cifStatespace_{min}})$ performs Petri Net synthesis to compute a minimal free-choice Petri Net whose behavior is trace equivalent to the single automaton in the given CIF specification, \var{cifStatespace_{min}}.
...
In our synthesis algorithm, we use PNML\footnote{See \url{https://pnml.lip6.fr}.} as the intermediate format for representing Petri Nets.
Therefore, part of the \keyw{petrinet-synthesis} operation is to translate \var{cifStatespace_{min}} to the input language of Petrify, and transforming the output of Petrify to PNML.
...
Therefore, the implementation of \keyw{petrinet-synthesis} also required recovering this region mapping, at least as long a Petrify is being used.
...
The operation $\keyw{reduce-state-annotations}(\var{cifStatespace})$ removes all state annotations, $\texttt{@state(\dots)}$, from locations in the given CIF state space, \var{cifStatespace}, in which a nondeterministic action is being executed.
