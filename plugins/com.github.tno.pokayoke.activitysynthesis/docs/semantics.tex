...
Guards are defined to be state predicates, while effects defined to be state transformers, i.e., functions that map states to any number of `successor states', to be able to model nondeterministic action behavior.

Let $\Guard = \State \rightarrow \Bool$ be the set of all \emph{guards} and $\Effect = \State \rightarrow 2^{\State}$ be the set of all \emph{effects} over states, with $2^{\State}$ the powerset of $\State$.
We use $g \in \Guard$ and $u \in \Effect$ to denote a typical guard and effect, respectively.

\paragraph{Actions.}
The main building blocks of activities are the action nodes, which are nodes that carry out a certain \emph{action}.
Any action consists of three parts: an action label to identify it, a guard that must hold in order for the action to be performed, and an effect that determines possible successor states after having performed the action.
For any action it holds that, if its guard does not hold, or if there is no successor state determined by the effect (i.e., the effect maps to an empty set), then the action cannot be performed.
In case action execution leads to multiple possible successor states, one of them will be selected nondeterministically, which is further explained in \autoref{sec:abstrdynamics}.

More formally, let $\Action = \cramped{\Label \times \Guard \times \Effect}$ be the set of all \emph{actions}, where $\Label$ is the set of all \emph{action labels}.
We use $a \in \Label$ to denote a typical label, and $\alpha \in \Action$ to denote a typical action, such that $\alpha = (a, g, u)$.
So actions are triples consisting of an action label $a$, a guard $g$, and effect $u$.

Let $\actlabel : \Action \rightarrow \Label$, $\guard : \Action \rightarrow \Guard$, and $\effect : \Action \rightarrow \Effect$ be projection functions that give the label, guard, and effect of any action, respectively, so that:
\begin{mathpar}
	\actlabel((a, g, u)) = a
	\and
	\guard((a, g, u)) = g
...
Any edge $\abstredge \in \mathcal{E}$ so that $\abstredge = (\abstrnode,g,\abstrnode')$ consists of a \emph{source node} $\abstrnode \in \mathcal{N}$, a \emph{target node} $\abstrnode' \in \mathcal{N}$ and a \emph{guard} $g \in \Guard$.
...
Any edge $\abstredge$ is said to be \emph{enabled in $c$} if it has a token in $c$, i.e., if $\abstredge \in \Sigma$.
If $c$ is clear from the context, we may just say that $\abstredge$ is \emph{enabled}.
Note that the edge guards do not influence whether an edge is enabled or not.
Instead, edge guards restrict when an edge can \emph{receive} a token (as opposed to whether they can hold a token).
...
\begin{mathpar}
	\inferrule[And] {
		\incoming(\abstrnode) \subseteq \Sigma
		\\
		(\outgoing(\abstrnode) \setminus \incoming(\abstrnode)) \cap \Sigma = \emptyset
		\\
		\guard(\alpha)(\sigma)
		\\
		\sigma' \in \effect(\alpha)(\sigma)
		\\
		\outgoing(\abstrnode) = \outgoing(\abstrnode, \sigma')
	} {
		(\Sigma,\sigma) \xrightarrow{\abstrnode} ((\Sigma \setminus \incoming(\abstrnode)) \cup \outgoing(\abstrnode),\sigma')
	} { 
		\text{ for } \abstrnode = (\ell, \andnode{\alpha})
	}
	
	\inferrule[Or] {
		\abstredge \in \incoming(\abstrnode) \cap \Sigma
		\\
		\abstredge' \in \outgoing(\abstrnode, \sigma') \setminus (\Sigma \! \setminus \! \{\abstredge\})
		\\
		\guard(\alpha)(\sigma)
		\\
		\sigma' \in \effect(\alpha)(\sigma)
	} {
		(\Sigma,\sigma) \xrightarrow{\abstrnode} ((\Sigma \setminus \{\abstredge\}) \cup \{\abstredge'\}, \sigma')
	} { 
		\text{ for } \abstrnode = (\ell, \ornode{\alpha})
	}
\end{mathpar}
...
It requires: (1) all incoming edges into $\abstrnode$ to have a token; (2) none of the outgoing edges of $\abstrnode$ except self-loops to have a token, otherwise action execution might attempt to put more than one token on a single edge, which is not allowed; (3) the action guard $\guard(\alpha)$ to hold with respect to the current state $\sigma$; (4) a successor state $\sigma'$ to be available from the effect $\effect(\alpha)$ of $\alpha$; and (5) the guards of all outgoing edges to hold with respect to $\sigma'$.
If all five premises are met, \textsc{And} removes all tokens from $\incoming(\abstrnode)$ and puts tokens on $\outgoing(\abstrnode)$, making $\cramped{(\Sigma \setminus \incoming(\abstrnode)) \cup \outgoing(\abstrnode)}$ the new tokken arrangement.
...
It requires: (1) the existence of an incoming edge $\abstredge$ of $\abstrnode$ that is enabled; (2) the existence of an outgoing edge $\abstredge'$ of $\abstrnode$ that is not enabled unless it equals $\abstredge$, and whose guard holds with respect to $\sigma'$; (3) the guard of $\alpha$ to hold with respect to the current state $\sigma$; and (4) a successor state $\sigma'$ to be available from $\alpha$'s effect.
...
If all five premises are met, \textsc{Or} removes the token from $\abstredge$ and puts it on $\abstredge'$, making $\cramped{(\Sigma \setminus \{\abstredge\}) \cup \{\abstredge'\}}$ the new arrangement of tokens.
...
Action-typed nodes $\act(\alpha)$ closely relate to opaque actions in UML in the sense that their execution updates the current state according to $\alpha$ (possibly nondeterministically so in case $\alpha$'s effect has multiple successor states to choose from).
...
Let $\denotn{~\cdot~}$ be defined as follows, where $\lambda\sigma.\true$ is the constant guard that is always $\true$, and $\identity{\sigma}$ is the identity effect that does not change the state:
\begin{mathpar}
...
Recall that the current definitions abstract over details like variables and their values\footnote{We are not going to define a custom expression/effect language and its semantics. There are plenty of good existing languages that could be picked off-the-shelf, like for example CIF.}.
...
\begin{itemize}
	...
	In particular, this means that $\effect(\alpha)$ is executed without interference from effects of other nodes.
...
\begin{tabular}{cl}
	$\AbstrNode$ & The set of all abstract activity nodes \\
	$\AbstrNodeType$ & The set of all abstract activity node types \\
	...
	$a$ & An action label \\
	$\alpha$ & An action, consisting of an action label, a guard and an effect \\
	...
	$\Effect$ & The set of all effects \\
	$\effect$ & Projection function for obtaining the effect of an action \\
