...
Given any control flow $\mathit{cf}$ in the UML activity that goes from some decision node \var{D} to some node \var{N}, the guard for this control flow $\mathit{cf}$ can be computed as $\texttt{simplify}(\texttt{simplify}(\var{N_{cond}}, \var{N_{guard}}), \var{D_{pred}})$, where:
\begin{itemize}
	\item \var{N_{cond}} is the original condition for executing \var{N} as specified in the input UML model.
	In case \var{N} is an action node, \var{N_{cond}} is the original guard predicate of the action as defined in the input UML model.
	In case \var{N} is a final node, \var{N_{cond}} is the activity postcondition, i.e., the condition needed to `finalize' the activity.
	For all other types of activity nodes, the \var{N_{cond}} predicate is simply \texttt{true}, since no enabling condition is explicitly defined in the input UML for fork/join/decision/merge/initial nodes. 
	\item \var{N_{guard}} is the full synthesized condition predicate for \var{N}, under which \var{N} can be executed in the controlled system.
	In case \var{N} is an action node, \var{N_{guard}} is the full condition that is computed by data-based synthesis, under which the action is enabled in the controlled system.
	For all other types of activity nodes, the \var{N_{guard}} predicate is simply \texttt{true}, since data-based synthesis only computes extra conditions for controllable events, which correspond only to actions.
...
To elaborate further on guard computation; one might consider to simply use \var{N_{cond}} as the control flow guard for $\mathit{cf}$, without doing any simplification.
...
Moreover, \var{N_{cond}} may be expressed over internal state, like internal CIF variables for encoding occurrence constraints that keep track of the number of occurrences of actions, which should not end up in control flow guards.
It therefore makes sense to simplify \var{N_{cond}} by using any contextual information we have at hand, if \var{N_{cond}} happens to be non-trivial.
We can simplify \var{N_{cond}} with respect to the guard of \var{N} in case \var{N_{guard}} is non-trivial, since any restrictions covered by \var{N_{guard}} are already considered by the execution semantics of activities.
