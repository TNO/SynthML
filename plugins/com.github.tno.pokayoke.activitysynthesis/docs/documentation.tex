...
The \texttt{Flipper} opaque behavior's effect is to flip the boolean \texttt{bit} after it is initialized: its guard is \texttt{init}, whilst its effects consist of a single assignment \texttt{bit := not bit}. 
...
\begin{example*}[Running Example]
The translation of the UML \texttt{BitFlipper} model to CIF is shown in Algorithm~\ref{alg:bitflipper-cif}. In the CIF model, first the events are defined, either controllable (denoting the start of an action) or uncontrollable (usually describing a nondeterministic or external event).  
The \texttt{BitFlipper} CIF model provides two controllable events, \texttt{Flipper} and \texttt{ND\_BitInit} that relate to starting the opaque behaviors defined in the UML model. 
...
The activity precondition and postcondition are translated into algebraic boolean variables. Then we define the initial state predicate \texttt{initial} to be the algebraic \texttt{precondition} variable, to indicate the initial states for performing synthesis. Moreover, we define the marked state predicate \texttt{marked} to be the algebraic \texttt{postcondition} variable, to indicate that a postcondition state must always be reachable in the controlled system.
The auxiliary variable \texttt{\_\_activeAction} is used to connect the start and end events of nonatomic and/or nondeterministic actions, in this case \texttt{ND\_BitInit}.
Finally, 
...
\begin{example*}[Running example]
    ...
    Notice that the end event \texttt{\_\_end} has been split into two separate transitions that will be merged together (in fact, as a UML merge node) in a following step. Further, the final place \texttt{p5} is equipped with a self loop, which will also be removed in one of the post-processing steps. 
\end{example*}
...
\begin{example*}[Running Example]
    Finally, the synthesized activity is shown in Fig.~\ref{fig:synthesized-bitflipper}. The initial node is marked in blue, 
    ...
    If \texttt{bit} is true, the flow can go directly to the final node (shown with a blue border); otherwise, the flow goes through the \texttt{Flipper} action. The two \texttt{\_\_end} transitions have been translated into a merge node, and finally place \texttt{p5} is transformed into the final node. 
\end{example*}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/bitflipper_activitysynthesised.png}
    \caption{The synthesized \texttt{BitFlipper} activity.}
