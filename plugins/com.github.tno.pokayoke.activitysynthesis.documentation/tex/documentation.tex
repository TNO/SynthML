...
\begin{description}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			Optimality constraints\footnote{Note that optimality constraints are temporary, and intended to be removed later.} are roughly of the form `action $A$ must happen at least $M$ times and at most $N$ times' and thus limit the number of occurrences of some action in a to-be-synthesized activity.
	...
	The goal of activity synthesis is then to synthesize a concrete UML activity for the specified abstract UML activity, and updating the UML model by replacing the abstract UML activity by the concrete synthesizedones.
...
\begin{description}
	...
	Giving some state machine (or finite automaton), the goal of Petri Net synthesis is to synthesize a (minimal) Petri Net that is trace equivalent to the input state machine.
...
\begin{itemize}
	...
	Optimality constraints are roughly of the form `action $A$ must happen at least $M$ times and at most $N$ times'.
...
The reason is that Petrify does not have symbolic Petri Net synthesis algorithms.
...
\begin{itemize}
	...
	\item Unless the supervisor was empty (in which case the activity synthesis chain will have crashed already before having generated the state space), there is at least one initial location and at least one marked location.
