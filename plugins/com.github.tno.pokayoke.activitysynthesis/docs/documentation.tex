...
The UML model is composed of a single class ActiveClass, which contains two properties: 
...
two Opaque Behavior named \texttt{Flipper} and \texttt{ND\_BitInit}. 
The activity has precondition constraint set as \texttt{bit=false} and a postcondition constraint set as \texttt{bit=true}. 
The Flipper opaque behavior's scope is to flip the boolean bit after it is initialised: its guard is \texttt{init=true} , whilst its effects are composed of a single definition \texttt{bit:=not bit}. 
...
\begin{figure}
    ...
    \caption{The BitFlipper model visualised within the PokaYoke UML Designer environment.}
...
The boolean property bit is transformed into a discrete boolean \texttt{disc bool} variable bit, which is not initialised explicitly, whereas init is initialised as true. 
The activity precondition and postcondition are translated into algebraic boolean variables; the activity precondition is denoted as initial, while the postcondition is denoted as \texttt{marked} (identifying a final location).  
...
Finally, 
the CIF model denotes the flipper event as an edge with precondition \texttt{init=true} and \texttt{\_\_activeAction = 0}, and postcondition \texttt{bit:= not bit}.   
\end{example*}
...
\begin{example*}[Running Example]
The minimised CIF model is shown in Algorithm~\ref{alg:bitflipper-minimised-cif}.  Any PokaYoke CIF model has a \texttt{\_\_start} and an \texttt{\_\_end} events, denoting the start and end of the activity to be synthesised. 
...
\begin{example*}[Running Example]
    ...
    Petrify's location \texttt{p3} has been turned into a decision node, and the two outgoing edges are equipped with the guards coming from an uncontrollable event, i.e. the nondeterministic initialisation of \texttt{bit}.  
    If \texttt{bit} is true, the flow can go directly to the end node (shown with a blue border); otherwise, the flow goes through the flipper action. Petrify's two \texttt{\_\_end} transitions have been translated into a merge node, and finally location \texttt{p5} is transformed into the final node. 
\end{example*}
