...
\begin{enumerate}
    ...
    \item Right click on the project $\to$ Viewpoints Selection $\to$ Check `SynthML'. 
...
When a call behavior node represents a call to an activity, we may abstract the call by adding guard and effects to the node itself, located in the `SynthML' tab of the `Properties' view.
