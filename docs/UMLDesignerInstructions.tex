...
\footnotetext{A da atype can also be added as an `Packaged Element' by right clinking on the model $\to$ Packaged Element $\to$ Class. The same consideration regarding the interchange of owned types and packaged elements mentioned earlier are valid. Since a data types is usually considered as a type, it is advised to use the owned type option.}
...
Note that in the UML designer activities can also be added as a nested classifier. This is \emph{not} allowed in a Poka Yoke model. 
