...
Thus by doing so, for every activity node, we compute the extra guard that must hold to be able to safely execute that node. 
...
Whether an incoming or outgoing guard is computed, or whether this computed guard is put on an incoming or outgoing control flow of an activity node, is dependent on the type of the activity node.
...
However, this activity does not contain any data (other than the guards and effects of actions).
This is primarily because Petri Net synthesis is unable to deal with data.
...
\begin{itemize}
	...
	Moreover, we do not translate any other concrete activities that might be in the input model, since \var{umlActivity_{concr}} will not call any other activities since it is flattened.
	\item Two variants of the activity postcondition are now translated to CIF: one ``without structure'' and one ``with structure''.
	The ``without structure'' variant is the same as the postcondition translation as explained in \autoref{sec:transform-uml-to-cif}.
	The ``with structure'' variant is extra, and is equivalent to the ``without structure'' variant plus the extra condition that only the final control flow---the one that goes into the final UML node of \var{umlActivity_{concr}}---holds a token.
	...
	However, \var{umlActivity_{concr}} does not call other activities, as it is flattened.
	Our goal now is computing the extra conditions needed to safely execute the nodes of \var{umlActivity_{concr}}, hence we make all their start events controllable.
	By doing so, we are able to compute control flow guards as locally as possible.
...
And as a result of the $\texttt{simplify}$ operation, it may happen that $\texttt{extra}(\mathit{edge})$ becomes a predicate that does depend in internal state.
We could have avoided that, by eliminating all internal CIF variables from the BDD representations of $S$ and $S'$ using existential quantification.
By doing so, $\texttt{extra}(\mathit{edge})$ is guaranteed not to depend on internal state.
However, as a downside to this, it might not always be possible to compute extra guards.
Sometimes the only way of expressing the extra guard of $\mathit{edge}$ is via internal state.
The code implementation checks for such internal state dependencies.
The activity synthesis algorithm terminates exceptionally when this check finds such a dependency, resulting in an error message for the user.
The full definition of $\texttt{extra}(\mathit{edge})$ now becomes:
\begin{equation*}
	\begin{split}
...
% TODO explain the actual guard computation
...
% TODO I'm not sure about the 'tracing information' part. Do we really depend on that? Revisit + decide later.
