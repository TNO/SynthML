 ...
 * the actual UML element ('isAtomic', 'isDeterministic', 'isShadowed'), to the associated CIF events ('isStartAction'
 * if it is the start of a UML atomic or non-atomic action/behavior, 'effectIdx' represents the effect number if the CIF
 * event represents the end of a UML non-atomic or non-deterministic action) and whether the related transition pattern
 * in the Petri net has been merged or not ('isMerged').
 */
public class UmlElementInfo {
    ...
    public UmlElementInfo(RedefinableElement umlElement) {
        if (umlElement != null) {
            ...
            if (umlElement instanceof CallBehaviorAction cbAction) {
                // If shadowed call behavior, use the call behavior element attributes. Otherwise, use the called
                // element attributes.
     ...
     * is null or a control node, or if the UML element does not belong to the input activity (i.e. is part of another
     * activity previously synthesized) and it's not an opaque behavior (since opaque behaviors are defined in the UML
     * model and not in any specific activity).
     *
