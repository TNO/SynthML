...
\begin{algorithm}[t!]
	...
	\procedure{\synth{\var{umlModel}}}{
		...
		\var{umlModel_{red}} := \reduceNonAtomicPatterns{\var{umlModel}_{concr}}\;
		\tcp{Compute the control flow guards for the UML decision nodes.}
		\var{umlModel_{guards}} := \computeEdgeGuards{\var{umlModel_{red}}, \, \dots}\;
...
The operation $\reduceNonAtomicPatterns{\var{umlModel}_{concr}}$ merges any nonatomic patterns in the input UML model $\var{\var{umlModel}_{concr}}$ that can be merged.
...
\paragraph{Preconditions.}
This operation requires a UML model $\var{umlModel}_{concr}$ resulting from the previous step (the code implementation actually requires its earlier Petri Net representation).
...
\paragraph{Postconditions.}
This operation returns the input model, where all nonatomic patterns have been rewritten as shown on the right in \autoref{fig:non-atomic-pattern}.
...
The operation \computeEdgeGuards{\var{umlModel_{red}}, \, \dots} computes guards for every control flow in the single concretized UML activity in \var{umlModel_{red}} that goes out of a decision node.
...
\begin{itemize}
	...
	That is, $\mathit{controlled}(N)$ is the extra synthesized condition for the action called by $N$.
	\item In all other cases, $\mathit{controlled}(N)$ is undefined.
	That is, no extra conditions were synthesized that further restrict $N$ in the controlled system.
\end{itemize}
...
\paragraph{Preconditions.}
The input UML model \var{umlModel_{red}} should be the original UML model that was used as input to the activity synthesis algorithm, with its single abstract UML activity replaced by a concrete synthesized one (as result of \autoref{sec:transform-petrinet-to-activity}), and with its nonatomic patterns reduced wherever possible (as result of \autoref{sec:reducing-non-atomic-patterns}).
...
\paragraph{Postconditions.}
The output is a UML model that is equal to \var{umlModel_{red}}, but with proper choice guards added to all control flows that go out of decision nodes.
...
The operation \postprocessActivity{\var{umlModel_{guards}}} post-processes the synthesized activity in \var{umlModel_{guards}}, to:
\begin{itemize}
...
This is done for two reasons: (1) we know that translating a minimal Petri Net to an UML activity does not imply that the translated activity is minimal, and (2) the removal of internal actions may lead to patterns in the UML activity that may further be reduced.
