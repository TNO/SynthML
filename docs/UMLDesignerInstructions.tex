...
A snapshot of the main screen is shown in Fig.~\ref{fig:PY-environment}, with the main components highlighted. 
...
\begin{enumerate}
    ...
    This enables all the Poka Yoke features, such as the addition of guards and effects to opaque behaviors. 
\end{enumerate}
...
\end{enumerate}
Note that you still need to create an enumeration and integer variable via the standard approach before you can create an integer or enumeration property.
...
Data types represent more abstract types, \eg a robot. To create a data type, 
\begin{enumerate}
...
\footnotetext{A data type can also be added as an `Packaged Element' by right clicking on the model $\to$ Packaged Element $\to$ Data Type. The same consideration regarding the interchange of owned types and packaged elements mentioned earlier are valid. Since a data type is usually considered as a type, it is advised to use the owned type option.}
...
defining respectively the system states at the beginning and the end of the the activity execution. 
...
Note that in UML Designer activities can also be added as a nested classifier. This is \emph{not} allowed in a Poka Yoke model. 
...
Similarly to abstract activities, also concrete activities can be visualized via an Activity Diagram (see \S\ref{subsec:abstract-activity}). Further, concrete activities can also be added as a nested classifier. This is \emph{not} allowed in a Poka Yoke model. 
...
\begin{enumerate}
    ...
    \item In the `Properties' view, `Poka Yoke' tab, you can add guards and effects for the opaque action.
\end{enumerate} 
...
\item For atomic actions, in the `Properties' view, `Poka Yoke' tab, check the `Atomic' checkbox.
...
\begin{enumerate}
    ...
    The default value of \texttt{init} is false, none (leave an empty field) for \texttt{bit}.  
    \item Create an abstract activity, with precondition \verb#not bit#, 
    and postcondition \verb#bit#.
    \item Create an opaque behavior, named \texttt{ND\_BitInit}, that models an initialization  action, with guard \texttt{not init} and nondeterministic effects:
    \newline
...
\begin{figure}
    ...
    \caption{The activity diagram of the synthesized bit flipper.}
