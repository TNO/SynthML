...
However, as a downside to this, it might then not always be possible to compute extra guards.
...
\begin{itemize}
	...
	By doing so, $\mathit{umlNode}$ can only be executed if it can take a token from its single incoming control flow, to which we have added the extra guard.
	\item If $\mathit{umlNode}$ is a fork node, then $\texttt{extra}(\mathit{edge})$ will become the outgoing guard of the single incoming control flow of $\mathit{umlNode}$.
...
Finally, as a last step, to prevent BDD memory leaks, we free all BDDs that have not been freed by data-based synthesis, due to our configuration.
