...
\begin{description}
	...
	Note that occurrence constraints are temporary constructs and are intended to be removed later, which is also future work.
...
a boolean named \texttt{init}, initialized as false, denoting whether the system is initialized, and 
a boolean named \texttt{bit}, not initialized.  
The class also contains 
an abstract activity called \texttt{ActivityMain}, and 
two opaque behaviors named \texttt{flip} and \texttt{initialize}. 
...
The opaque behavior \texttt{initialize} turns \texttt{init} from false to true, and instantiates \texttt{bit} either as true of false in a nondeterministic fashion. 
...
In case the abstract activity may be able to call other concrete activities, then all nodes and control flows of these concrete activities are translated as well.
...
Moreover, every occurrence constraint of \var{umlActivity_{abstr}} must be expressed over actions and/or activities.
If the to-be-synthesized activity is able to call other activities by its occurrence constraints, then all these activities must be concrete, and are not allowed to call other activities (i.e., they must be flattened).
...
\begin{itemize}
	...
	\begin{itemize}
		\item Contains a Boolean-typed CIF discrete variable for every control flow of the concrete UML activity.
		This variable then indicates whether the corresponding control flow contains a token (\texttt{true}) or not (\texttt{false}).
		\item Contains CIF event declarations corresponding to all nodes of the concrete UML activity.
	...
	Furthermore, in case concrete activities were translated, extra postconditions are generated expressing that none of their control flows must have a token.
	...
	Thus, a marked state is a state where the (combined) user-written activity postcondition is satisfied, where no nonatomic and nondeterministic actions are active, where every occurrence constraint is satisfied, and where no translated control flow holds a token.
	...
	Occurrence constraints are roughly of the form `$A$ must happen at least $M$ times and at most $N$ times' where $A$ refers either to an action or an activity.
	...
	In case $A$ is an action, this variable is incremented every time the action occurs.
...
The activity precondition and postcondition are translated into algebraic boolean variables. Then we define the initial state predicate \texttt{not init} to be the algebraic \texttt{precondition} variable, to indicate the initial states for performing synthesis. Moreover, we define the marked state predicate \texttt{marked} to be the algebraic \texttt{postcondition} variable, to indicate that a postcondition state must always be reachable in the controlled system.
...
Finally, 
the CIF model denotes the \texttt{flip} event as an edge with guards \texttt{init} and \texttt{\_\_activeAction = 0}, and effects composed of the single update \texttt{bit := not bit}.   
\end{example*}
...
\begin{itemize}
    ...
    \item location $s4$ has two outgoing edges towards $s3$, after event \texttt{\_\_end} (in case \texttt{bit} is initialized as true), and towards $s5$ after event \texttt{flip} (otherwise); 
...
\begin{example*}[Running Example]
    ...
    If \texttt{bit} is true, the flow can go directly to the final node (shown with a blue border); otherwise, the flow goes through the \texttt{flip} action. The two \texttt{\_\_end} transitions have been translated into a merge node, and finally place \texttt{p5} is transformed into the final node. 
\end{example*}
