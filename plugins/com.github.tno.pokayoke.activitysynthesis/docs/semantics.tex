...
Abstract activity nodes consist of two parts: a node identifier, and a node type that also contains (wraps) the action to perform.
...
If all five premises are met, \textsc{And} removes all tokens from $\incoming(\abstrnode)$ and puts tokens on $\outgoing(\abstrnode)$, making $\cramped{(\Sigma \setminus \incoming(\abstrnode)) \cup \outgoing(\abstrnode)}$ the new token arrangement.
