...
More formally, core activities $\mathcal{A} = (\mathcal{N},\mathcal{E})$ are graphs with nodes $\mathcal{N} \subseteq \CoreNode$ and edges $\mathcal{E} \subseteq \cramped{\mathcal{N} \times \Guard \times \mathcal{N}}$.
Any edge $\coreedge \in \mathcal{E}$ so that $\coreedge = (\corenode,g,\corenode')$ consists of a \emph{source node} $\corenode \in \mathcal{N}$, a \emph{target node} $\corenode' \in \mathcal{N}$ and a \emph{guard} $g \in \Guard$.
...
\begin{itemize}
	...
	\item $\incoming(\corenode,\sigma) = \{ \coreedge \in \incoming(\eta) ~|~ \guard(\coreedge)(\sigma) \}$ be all incoming edges of $\corenode$ whose guard holds with respect to $\sigma$.
	\item $\outgoing(\corenode,\sigma) = \{ \coreedge \in \outgoing(\eta) ~|~ \guard(\coreedge)(\sigma) \}$ be all outgoing edges of $\corenode$ whose guard holds with respect to $\sigma$.
\end{itemize}
...
Any edge $\coreedge$ is said to be \emph{enabled in $c$} if it has a token in $c$, i.e., if $\coreedge \in \Sigma$.
...
\begin{mathpar}
	...
	\inferrule[Or] {
		\coreedge \in \incoming(\corenode) \cap \Sigma
		\\
...
If all five premises are met, \textsc{Or} removes the token from $\coreedge$ and puts it on $\coreedge'$, making $\cramped{(\Sigma \setminus \{\coreedge\}) \cup \{\coreedge'\}}$ the new arrangement of tokens.
...
Let $\denote{~\cdot~} : \ConcrNode \times \Guard \times \ConcrNode \rightarrow \CoreNode \times \Guard \times \CoreNode$ be the translation function for edges, defined such that
