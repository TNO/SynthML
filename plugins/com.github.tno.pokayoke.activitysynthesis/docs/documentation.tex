...
Finally, 
the CIF model denotes the \texttt{Flipper} event as an edge with guards \texttt{init} and \texttt{\_\_activeAction = 0}, and effects composed of the single update \texttt{bit := not bit}.   
\end{example*}

\input{images/bitflipper-cif}
...
\begin{itemize}
    ...
    \item location $s2$ has one outgoing edge towards $s4$, after event \texttt{ND\_BitInit}; 
    \item location $s3$ is marked, namely is a final location, hence without outgoing edges; 
    \item location $s4$ has two outgoing edges towards $s3$, after event \texttt{\_\_end} (in case \texttt{bit} is initialized as true), and towards $s5$ after event \texttt{Flipper} (otherwise); 
    \item location $s5$ has one outgoing edge towards $s3$.
\end{itemize}
This state space is the input to Petrify, that synthesizes a Petri Net from it. 
\end{example*}

\input{images/bitflipper-synthesised-minimized}
...
\begin{example*}[Running Example]
    ...
    the place \texttt{p3} has been turned into a decision node, and the two outgoing control flows represent the nondeterministic initialization of \texttt{bit}.  
