...
a boolean named \texttt{init}, initialized as \texttt{false}, denoting whether the system is initialized, and 
...
The auxiliary variable \texttt{\_\_activeAction} is used to connect the start and end events of nondeterministic actions, in this case \texttt{initialize}.
The \texttt{flip} event is translated into an edge with guards \texttt{init} and \texttt{\_\_activeAction = 0}, and effects composed of the single update \texttt{bit := not bit}.   
The opaque behavior \texttt{initialize} is translated into a series of edges. 
First, an edge \texttt{initialize} is defined, with guard \texttt{init = false and \_\_activeAction = 0}, whose effect is \texttt{\_\_activeAction := 1} (representing the nondeterministic action is being performed). 
...
At the same time, both edges assign \texttt{true} to \texttt{init} and \texttt{0} to \texttt{\_\_activeAction}, representing respectively the initialization of the system, and the end of the nondeterministic action. 
Finally, four requirement invariants state that the postcondition disables all CIF events. 
In practice, these four requirements mimic the fact that once the system reaches a state where the postcondition holds, no additional actions need to be taken. 
\end{example*}

\ciffile{images/bitflipper_cif.cif}{CIF translation of the \texttt{BitFlipper} UML model.}{alg:bitflipper-cif}
...
\begin{example*}[Running Example]
The minimized CIF model is shown in Algorithm~\ref{alg:bitflipper-minimized-cif}. The CIF model has a \texttt{\_\_start} and an \texttt{\_\_end} event, denoting the start and end of the activity to be synthesized. 
...
\ciffile{images/bitflipper_synthesized_minimized.cif}{Minimized CIF model.}{alg:bitflipper-minimized-cif}
...
\begin{figure}[t]
	\begin{center}
		\includegraphics[scale=0.6]{images/non_atomic_pattern}
	\end{center}
