...
two opaque behaviors named \texttt{Flipper} and \texttt{initialize}. 
...
The \texttt{initialize} turns \texttt{init} from false to true, and instantiates \texttt{bit} either as true of false in a nondeterministic fashion. 
...
\begin{figure}
    ...
    \caption{The \texttt{flip} model visualized within the PokaYoke UML Designer environment.}
...
\begin{example*}[Running Example]
The translation of the UML \texttt{flip} model to CIF is shown in Algorithm~\ref{alg:bitflipper-cif}. In the CIF model, first the events are defined, either controllable (denoting the start of an action) or uncontrollable (usually describing a nondeterministic or external event).  
The \texttt{flip} CIF model provides two controllable events, \texttt{Flipper} and \texttt{initialize} that relate to starting the opaque behaviors defined in the UML model. 
...
The auxiliary variable \texttt{\_\_activeAction} is used to connect the start and end events of nonatomic and/or nondeterministic actions, in this case \texttt{initialize}.
Finally, 
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/bitflipper.png}
...
\begin{figure}
    ...
    \caption{The synthesized \texttt{flip} activity.}
