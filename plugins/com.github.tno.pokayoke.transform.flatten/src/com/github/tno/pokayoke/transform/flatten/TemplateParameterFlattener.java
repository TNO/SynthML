...
public class TemplateParameterFlattener {
     ...
     * @param argumentMap The mapping of template parameter names to concrete arguments.
     */
    private static void unfoldRedefinableElement(RedefinableElement element, Map<String, AExpression> argumentMap) {
        // Perform the guard unfolding. Skip if there is no guard.
        AExpression guardExpr = CifParserHelper.parseGuard(element);
        if (guardExpr != null) {
            AExpression newGuard = unfoldAExpression(guardExpr, argumentMap);
        ...
        for (String effect: effects) {
            ...
            String newEffect = updates.stream().flatMap(update -> unfoldAUpdate(update, argumentMap).stream())
     ...
     */
    private static AExpression unfoldAExpression(AExpression expression, Map<String, AExpression> argumentMap) {
        if (expression instanceof ABinaryExpression binExpr) {
            AExpression unfoldedLhsExpression = unfoldAExpression(binExpr.left, argumentMap);
            AExpression unfoldedRhsExpression = unfoldAExpression(binExpr.right, argumentMap);
            ...
            return new AUnaryExpression(unaryExpr.operator, unfoldAExpression(unaryExpr.child, argumentMap),
                    unaryExpr.position);
        } else if (expression instanceof ANameExpression nameExpr) {
            return argumentMap.getOrDefault(nameExpr.name.name, nameExpr);
            ...
            List<AExpression> guardExprs = ifExpr.guards.stream().map(guard -> unfoldAExpression(guard, argumentMap))
                    .toList();
            AExpression thenExpr = unfoldAExpression(ifExpr.then, argumentMap);
            List<AElifExpression> elifExprs = ifExpr.elifs.stream()
                    .map(elif -> unfoldAElifExpression(elif, argumentMap)).toList();
            AExpression elseExpr = unfoldAExpression(ifExpr.elseExpr, argumentMap);
    ...
    private static AElifExpression unfoldAElifExpression(AElifExpression elifExpr,
            Map<String, AExpression> argumentMap)
    {
        List<AExpression> guardExprs = elifExpr.guards.stream().map(guard -> unfoldAExpression(guard, argumentMap))
                .toList();
        AExpression thenExpr = unfoldAExpression(elifExpr.then, argumentMap);
     ...
     */
    private static List<AUpdate> unfoldAUpdate(AUpdate update, Map<String, AExpression> argumentMap) {
        if (update instanceof AAssignmentUpdate assign) {
            return unfoldAAssignmentUpdate(assign, argumentMap);
        } else if (update instanceof AIfUpdate ifUpdate) {
            AUpdate newIfUpdate = unfoldAIfUpdate(ifUpdate, argumentMap);
            ...
            Map<String, AExpression> argumentMap)
    {
        ...
        Verify.verify(assignUpdate.addressable instanceof ANameExpression adressable
                && !argumentMap.containsKey(adressable.name.name));

        return List.of(new AAssignmentUpdate(assignUpdate.addressable,
                unfoldAExpression(assignUpdate.value, argumentMap), assignUpdate.position));
    }
     ...
     */
    private static AUpdate unfoldAIfUpdate(AIfUpdate ifUpdate, Map<String, AExpression> argumentMap) {
        // Process the 'if' statements.
        List<AExpression> unfoldedGuards = ifUpdate.guards.stream().map(u -> unfoldAExpression(u, argumentMap))
                .toList();

        // Process the 'then' statements.
        List<AUpdate> unfoldedThens = ifUpdate.thens.stream().flatMap(u -> unfoldAUpdate(u, argumentMap).stream())
                .toList();

        // Process the 'elif' statements.
        List<AElifUpdate> unfoldedElifs = ifUpdate.elifs.stream().map(u -> unfoldAElifUpdate(u, argumentMap)).toList();

        // Process the 'else' statements.
        List<AUpdate> unfoldedElses = ifUpdate.elses.stream().flatMap(u -> unfoldAUpdate(u, argumentMap).stream())
                .toList();

        return new AIfUpdate(unfoldedGuards, unfoldedThens, unfoldedElifs, unfoldedElses, ifUpdate.position);
    }

    private static AElifUpdate unfoldAElifUpdate(AElifUpdate elifUpdate, Map<String, AExpression> argumentMap) {
        // Process the 'guards'.
        List<AExpression> unfoldedElifGuards = elifUpdate.guards.stream().map(u -> unfoldAExpression(u, argumentMap))
                .toList();

        // Process the 'thens'.
        List<AUpdate> unfoldedElifThens = elifUpdate.thens.stream().flatMap(u -> unfoldAUpdate(u, argumentMap).stream())
                .toList();
    ...
    public static void unfoldActivity(Activity activity, CallBehaviorAction callBehaviorActionToReplace) {
        ...
        Map<String, AExpression> argumentMap = activityArguments.stream().collect(
        ...
        for (Element ownedElement: activity.getOwnedElements()) {
            if (ownedElement instanceof ControlFlow controlEdge) {
                ...
                if (incomingGuard != null) {
                    AExpression unfoldedIncoming = unfoldAExpression(incomingGuard, argumentMap);
                ...
                if (outgoingGuard != null) {
                    AExpression unfoldedOutgoing = unfoldAExpression(outgoingGuard, argumentMap);
                ...
                if (PokaYokeUmlProfileUtil.isFormalElement(callBehavior)) {
                    ...
                    unfoldRedefinableElement(callBehavior, argumentMap);
                }
