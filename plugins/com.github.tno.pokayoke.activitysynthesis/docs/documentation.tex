...
However, we have not yet validated this, so we are unsure whether this is the best fixpoint order strategy in general.
...
BDDs are used (by data-based synthesis but also guard computation) to symbolically represent and conveniently manupulate sets of system states.
Since guards are predicates over the system state, they can be represented as BDDs.
We will compute control flow guards on BDD level, after which we convert them to CIF expressions.
