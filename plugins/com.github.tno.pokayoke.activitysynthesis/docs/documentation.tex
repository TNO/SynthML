...
% Colored keywords for algorithm 
\usepackage{xcolor}
...
{
    keywords=[1]{
        controllable, uncontrollable, plant, automaton, disc, bool, any, true, false, int, alg, bool, initial, marked, location, initial, edge, when, do, end, requirement, invariant, disables
    },
    ...
    keywords=[4]{@state},
    keywordstyle=[4]\color{Grey},
    sensitive=false,
    morestring=[b]',
    morecomment=[l]{--}, 
    morecomment=[s][\color{red!50!brown}\ttfamily]{"}{"}, 
}
...
\begin{description}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			Occurrence constraints thus limit the number of occurrences of some action, or calls to some activity, in a to-be-synthesized activity.
	...
	Both abstract and concrete UML activities may contain preconditions, postconditions, and occurrence constraints.
	...
	In the case of concrete activities, these constraints are added primarily as documentation, but they might later be used for, e.g., property checking or for synthesizing activities that can call other activities.
	The preconditions and postconditions are state invariants that are expressed over the specified UML properties.
	Occurrence constraints are roughly of the form `$A$ must happen at least $M$ times and at most $N$ times' where $A$ can be either an action or an activity.
	Occurrence constraints thus limit the number of occurrences of some action, or calls to some activity, in a to-be-synthesized activity.
	At the moment, the synthesis algorithm does not yet support synthesis of activities that can call other activities.
	Nevertheless, the UML models that we consider may already have occurrence constraints over activities.
	Occurrence constraints are temporary constructs and are intended to be removed later, which is also future work.
...
Let us consider a simple UML model, where we aim at synthesizing an activity that flips a single nondeterministically initialized boolean value.  
The UML model is composed of a single class \texttt{ActiveClass}, which contains two properties: 
a boolean named \texttt{init}, initialized as false, denoting whether the bit is initialized, and 
a boolean named \texttt{bit}, not initialized.  
The class also contains 
an abstract activity called \texttt{ActivityMain}, and 
two Opaque Behaviors named \texttt{Flipper} and \texttt{ND\_BitInit}. 
The activity has precondition \texttt{not bit}, and postcondition \texttt{bit}. 
The \texttt{Flipper} opaque behavior's scope is to flip the boolean \texttt{bit} after it is initialized: its guard is \texttt{init}, whilst its effects consist of a single assignment \texttt{bit := not bit}. 
The \texttt{ND\_BitInit} turns \texttt{init} from false to true, and instantiates \texttt{bit} either as true of false in a nondeterministic fashion. 
...
Moreover, all occurrence constraints of \var{umlActivity_{abstr}} must be expressed over actions, i.e., they may not (yet) be expressed over activities.
...
\begin{itemize}
	...
	There may be multiple user-defined postconditions defined for this activity.
	...
	Moreover, in case the abstract activity has occurrence constraints, extra postconditions are generated which express that that occurrence constraints must be satisfied, i.e., the actions/activities that are subject to occurrence constraints must have happened the specified number of times.
	...
	Thus, a marked state is a state where the (combined) user-written activity postcondition is satisfied, and where no nonatomic and nondeterministic actions are active.
	...
	(Note that $A$ must be an action since we don't yet allow occurrence constraints to be expressed over activities.)
...
\begin{example*}[Running Example]
The translation to CIF of the \texttt{BitFlipper} model is shown in Algorithm~\ref{alg:bitflipper-cif}. The CIF model first defines the events, either controllable (denoting the start of an action) or uncontrollable (usually describing a nondeterministic or external event).  
The \texttt{BitFlipper} model provides two controllable events, \texttt{Flipper} and \texttt{ND\_BitInit} that relate to the opaque behaviors defined in the UML model. 
Two uncontrollable events denote the two nondeterministic result of the \texttt{bit} initialisation. 
The boolean property \texttt{bit} is transformed into a discrete boolean \texttt{disc bool} variable, which is not initialized explicitly, whereas \texttt{init} is initialized as false. 
The activity precondition and postcondition are translated into algebraic boolean variables. Then we define the initial state predicate \texttt{initial} to be the algebraic \texttt{precondition} variable, to indicate that this is the initial state for performing synthesis. Moreover, we define the marked state predicate \texttt{marked} to be the algebraic \texttt{postcondition} variable, to indicate that a postcondition state must always be reachable in the controlled system.
The auxiliary variable \texttt{\_\_activeAction} is used to split the two nondeterministic results of the initialisation of \texttt{bit}. 
Finally, 
the CIF model denotes the \texttt{Flipper} event as an edge with precondition \texttt{init=true} and \texttt{\_\_activeAction = 0}, and postcondition \texttt{bit := not bit}.   
\end{example*}

\input{bitflipper-cif}
...
\begin{example*}[Running Example]
The minimized CIF model is shown in Algorithm~\ref{alg:bitflipper-minimised-cif}. The CIF model has a \texttt{\_\_start} and an \texttt{\_\_end} event, denoting the start and end of the activity to be synthesized. 
The CIF automaton is composed of five states: 
\begin{itemize}
    \item state $s1$ is the initial state, with one outgoing edge towards $s2$ after event \texttt{\_\_start};
    \item state $s2$ has one outgoing edge towards $s4$, after event \texttt{ND\_BitInit}; 
    \item state $s3$ is marked, namely is a final state, hence without outgoing edges; 
    \item state $s4$ has two outgoing edges towards $s3$, after event \texttt{\_\_end} (in case \texttt{bit} is initialized as true), and towards $s5$ after event \texttt{Flipper} (otherwise); 
    \item state $s5$ has one outgoing edge towards $s3$.
\end{itemize}
This state space is the input to Petrify, that synthesized a Petri Net from it. 
\end{example*}

\input{bitflipper-synthesised-minimised}
...
\begin{example*}[Running example]
    The petrify output is depicted in Fig.~\ref{fig:petrify-out}. 
    Notice that the end event \texttt{\_\_end} has been split into two separate events that will be merged together (in fact, as a merge node) in a following step. Further, the final location \texttt{p5} is equipped with a self loop, which will also be removed in one of the post-processing steps. 
\end{example*}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/bitflipper.dot.png}
...
\begin{example*}[Running Example]
    Finally, the activity synthesized is shown in Fig.~\ref{fig:synthesized-bitflipper}. The start node is marked in blue, 
    Petrify's location \texttt{p3} has been turned into a decision node, and the two outgoing control flows are equipped with the guards coming from an uncontrollable event, i.e. the nondeterministic initialisation of \texttt{bit}.  
    If \texttt{bit} is true, the flow can go directly to the end node (shown with a blue border); otherwise, the flow goes through the \texttt{Flipper} action. Petrify's two \texttt{\_\_end} transitions have been translated into a merge node, and finally place \texttt{p5} is transformed into the final node. 
\end{example*}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/bitflipper_activitysynthesised.png}
    \caption{The \texttt{BitFlipper} activity synthesized.}
