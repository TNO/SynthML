...
The class also contains an abstract activity called \texttt{ActivityMain}, and two opaque behaviors named \texttt{flip} and \texttt{initialize}. 
...
\begin{example*}[Running Example]
The translation of the UML \texttt{BitFlipper} model to CIF is shown in Listing~\ref{alg:bitflipper-cif}. 
...
Then we define the initial state predicate \texttt{not init} to be the algebraic \texttt{precondition} variable, to indicate the initial states for performing synthesis. 
Moreover, we define the marked state predicate \texttt{marked} to be the algebraic \texttt{postcondition} variable, to indicate that a postcondition state must always be reachable in the controlled system.
...
\begin{example*}[Running Example]
The minimized CIF model is shown in Listing~\ref{alg:bitflipper-minimized-cif}. 
...
Notice that the end event \texttt{\_\_end} has been split into two separate transitions that will be merged together (in fact, as a UML merge node) in a following step. 
Further, the final place \texttt{p5} is equipped with a self loop, which will also be removed in one of the post-processing steps. 
\end{example*}
...
If \texttt{bit} is true, the flow can go directly to the final node (shown with a blue border); otherwise, the flow goes through the \texttt{flip} action. 
