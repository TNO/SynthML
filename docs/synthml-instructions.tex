...
A UML class represents a (sub)system that has state and behaviors. 
Every class \emph{must} have a classifier behavior, which must be an activity, either abstract (see \S\ref{subsec:abstract-activity}) or concrete (\S\ref{subsec:concrete-activity}).
An active class starts its classifier behavior as a direct consequence of its creation.
...
Properties represent the variables of a model.
The supported types are booleans, enumerations and ranged integers (besides composite data types, see \S\ref{subsec:data-types}).
...
To be able to synthesize a meaningful activity, we should define preconditions (see \S\ref{subsubsec:precondition}) and postconditions (see \S\ref{subsubsec:postcondition}), defining respectively the system states at the beginning and the end of the activity execution.
When preconditions or postconditions are not defined, they default to `true'.
We support two kinds of preconditions, namely \emph{synthesis} preconditions and \emph{usage} preconditions. 
The synthesis preconditions are used by the synthesis algorithm to create the UML activity; usage preconditions are considered when the synthesized activity is called by another activity.

The synthesis preconditions represent the ideal system state at the beginning of the activity.
For instance, consider synthesizing an activity that transfers a product from station A to station B.
A natural breakdown of this activity might be: there is a product at station A, a robot moves to station A, picks up the product, travels to station B (which is empty and ready), and finally releases the product.
This represents an idealized scenario.
What happens if station B is not empty at the beginning of this activity? 
Should the activity wait until station B is available (a clearly sub-optimal solution), or should it also support this case?
This is where usage preconditions come into play.
They allow the synthesized activity to operate under a different set of initial condition than those assumed during the synthesis phase, taking into account that this called activity may run concurrently with other activities.
These preconditions define a more common use of the activity: in this case, the usage precondition considers station B to be either empty or busy, while the synthesis preconditions assume an empty station B.
...
\begin{enumerate}
\item Right click on the activity $\to$ New Child $\to$ Precondition $\to$ Constraint.
\item Click on the `SynthML' tab, where you can specify the precondition name, kind (choose between `Synthesis precondition' and `Usage precondition') and the precondition expression. 
\end{enumerate}
...
\begin{enumerate}
\item Right click on the activity $\to$ New Child $\to$ Postcondition $\to$ Constraint.
\item Click on the `SynthML' tab, where you can specify the postcondition name, kind (choose `Postcondition') and the postcondition expression. 
\end{enumerate}
...
The supported types are booleans, enumerations and ranged integers.
...
Only literals and parameters of the calling activity can be passed as arguments.
...
\begin{enumerate}
    ...
    \item Click on the `SynthML' tab, where you can specify the constraint name, kind (choose `Requirement') and the constraint expression. The expression body can be several things:
    \begin{itemize}
...
\begin{enumerate}
    ...
    \item Create an abstract activity as the `ActiveClass' classifier behavior, with synthesis precondition \verb#not init#, and postcondition \verb#bit and init#.
