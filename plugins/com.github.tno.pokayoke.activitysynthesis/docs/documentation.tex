...
\begin{description}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Abstract UML activities are `empty' in the sense that they contain no nodes nor control flow.
			...
			Occurrence constraints are roughly of the form `$A$ must happen at least $M$ times and at most $N$ times' where $A$ can be either an activity or an action.
			...
			At the moment, the implementation of the synthesis algorithm does not yet support synthesis of activities that can call other activities.
			Nevertheless, the UML models that we consider may already have occurrence constraints over activities.
			Occurrence constraints are temporary constructs, and intended to be removed later, which is also future work.
