...
Two uncontrollable events denote the two nondeterministic result of the \texttt{bit} initialization. 
...
The activity precondition and postcondition are translated into algebraic boolean variables. Then we define the initial state predicate \texttt{initial} to be the algebraic \texttt{precondition} variable, to indicate that this is the initial state for performing synthesis, i.e., the ones satisfying \texttt{precondition}. Moreover, we define the marked state predicate \texttt{marked} to be the algebraic \texttt{postcondition} variable, to indicate that a postcondition state must always be reachable in the controlled system.
The auxiliary variable \texttt{\_\_activeAction} is used to split the two nondeterministic results of the initialization of \texttt{bit}. 
Finally, 
the CIF model denotes the \texttt{Flipper} event as an edge with guard \texttt{init} and \texttt{\_\_activeAction = 0}, and effects composed of the single update \texttt{bit := not bit}.   
\end{example*}
...
\begin{example*}[Running Example]
    ...
    the location \texttt{p3} has been turned into a decision node, and the two outgoing control flows represent the nondeterministic initialization of \texttt{bit}.  
    If \texttt{bit} is true, the flow can go directly to the end node (shown with a blue border); otherwise, the flow goes through the \texttt{Flipper} action. The two \texttt{\_\_end} transitions have been translated into a merge node, and finally place \texttt{p5} is transformed into the final node. 
\end{example*}
