...
Let us consider a simple UML model, where we aim at synthesising an activity that flips a single nondeterministically initialised boolean value.  
The UML model is composed of a single class \texttt{ActiveClass}, which contains two properties: 
a boolean named \texttt{init}, initialised as false, denoting whether the bit is initialised, and 
a boolean named \texttt{bit}, not initialised.  
...
The \texttt{Flipper} opaque behavior's scope is to flip the boolean \texttt{bit} after it is initialised: its guard is \texttt{init} , whilst its effects consist of a single assignment \texttt{bit:=not bit}. 
...
\begin{figure}
    ...
    \caption{The \texttt{BitFlipper} model visualised within the PokaYoke UML Designer environment.}
...
\begin{example*}[Running Example]
The translation to CIF of the BitFlipper model is shown in Algorithm~\ref{alg:bitflipper-cif}. The CIF model first defines the events, either controllable (denoting a possible choice) or uncontrollable (usually describing a nondeterministic or external event).  
...
The boolean property \texttt{bit} is transformed into a discrete boolean \texttt{disc bool} variable, which is not initialised explicitly, whereas \texttt{init} is initialised as true. 
...
The auxiliary variable \texttt{\_\_activeAction} is used to split the two nondetrministic results of the initialisation of \texttt{bit}. 
Finally, 
...
\begin{example*}[Running Example]
The minimised CIF model is shown in Algorithm~\ref{alg:bitflipper-minimised-cif}. The CIF model has a \texttt{\_\_start} and an \texttt{\_\_end} event, denoting the start and end of the activity to be synthesised. 
The CIF automaton is composed of 5 states: 
\begin{itemize}
    ...
    \item state $s4$ has two outgoing edges towards $s3$, after event \texttt{\_\_end} (in case \texttt{bit} is initialised as true), and towards $s5$ after event \texttt{Flipper} (otherwise); 
...
\begin{example*}[Running Example]
    Finally, the activity synthesised is shown in Fig.~\ref{fig:synthesised-bitflipper}. The start node is marked in blue, 
...
\begin{figure}
    ...
    \caption{The BitFlipper activity synthesised.}
    \label{fig:synthesised-bitflipper}
\end{figure}
