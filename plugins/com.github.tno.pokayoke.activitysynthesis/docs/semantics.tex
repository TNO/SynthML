...
The strategy for defining the operational semantics of activities is to first define a notion of \emph{core activities} (\autoref{sec:coreactivities}) with an intuitive and concise semantics, and then define the semantics of concrete UML activities by means of translating them to core activities (\autoref{sec:concractivities}).
...
\label{sec:coreactivities}

This section defines the (static) structure and (dynamic) behavior of core activities, in \autoref{sec:corestatics} and \autoref{sec:coredynamics}, respectively.

\subsection{Statics}
\label{sec:corestatics}

This section defines the static structure of core activities, consisting of nodes and (guarded) edges.
...
In case action execution leads to multiple possible successor states, one of them will be selected nondeterministically, which is further explained in \autoref{sec:coredynamics}.
...
\paragraph{Core activities.}
Let us now define \emph{core activities}, which are essentially graphs consisting of core nodes, as defined in the previous paragraph, that are connected via guarded edges.
The edges of core activities have two guards, namely an \emph{incoming guard} and an \emph{outgoing guard}.
The incoming guards are (later) used to restrict when an edge can \emph{receive} a token, while outgoing guards restrict when a token can be \emph{taken} from an edge.
That is, during activity execution, any edge can receive a token only when the activity is in a state where the incoming edge guard holds, and this token can be taken from the edge only when the outgoing edge guard holds.
Incoming edge guards are for example needed to define the semantics of UML decision nodes, of which an outgoing edge can only receive a token if its guard holds.
Outgoing edge guards are for example needed for defining extra (waiting) conditions for a following action to be performed.

More formally, core activities $\mathcal{A} = (\mathcal{N},\mathcal{E})$ are graphs with nodes $\mathcal{N} \subseteq \CoreNode$ and edges $\mathcal{E} \subseteq \cramped{\mathcal{N} \times \Guard \times \Guard \times \mathcal{N}}$.
Any edge $\coreedge \in \mathcal{E}$ so that $\coreedge = (\corenode,g_{\mathit{in}},g_{\mathit{out}},\corenode')$ consists of a source node $\corenode \in \mathcal{N}$, an incoming guard $g_{\mathit{in}} \in \Guard$, an outgoing guard $g_{\mathit{out}} \in \Guard$, and a target node $\corenode' \in \mathcal{N}$.
Let us define the following projection functions for edges:
\begin{itemize}
	\item $\source : \mathcal{E} \rightarrow \mathcal{N}$ such that $\source((\corenode,g_{\mathit{in}},g_{\mathit{out}},\corenode')) = \corenode$.
	\item $\incomingguard : \mathcal{E} \rightarrow \Guard$ such that $\incomingguard((\corenode,g_{\mathit{in}},g_{\mathit{out}},\corenode')) = g_{\mathit{in}}$.
	\item $\outgoingguard : \mathcal{E} \rightarrow \Guard$ such that $\outgoingguard((\corenode,g_{\mathit{in}},g_{\mathit{out}},\corenode')) = g_{\mathit{out}}$.
	\item $\target : \mathcal{E} \rightarrow \mathcal{N}$ such that $\target((\corenode,g_{\mathit{in}},g_{\mathit{out}},\corenode')) = \corenode'$.
\end{itemize}
We define the following shorthand notations for any $\corenode \in \mathcal{N}$ and $\sigma \in \State$:
\begin{itemize}
	\item $\incoming(\corenode) = \{ \coreedge \in \mathcal{E} ~|~ \target(\coreedge) = \corenode \}$ be the set of all incoming edges of $\corenode$.
	\item $\outgoing(\corenode) = \{ \coreedge \in \mathcal{E} ~|~ \source(\coreedge) = \corenode \}$ be the set of all outgoing edges of $\corenode$.
	\item $\incoming(\corenode,\sigma) = \{ \coreedge \in \incoming(\eta) ~|~ \outgoingguard(\coreedge)(\sigma) \}$ be all incoming edges of $\corenode$ whose outgoing guard holds with respect to $\sigma$.
	\item $\outgoing(\corenode,\sigma) = \{ \coreedge \in \outgoing(\eta) ~|~ \incomingguard(\coreedge)(\sigma) \}$ be all outgoing edges of $\corenode$ whose incoming guard holds with respect to $\sigma$.
\end{itemize}

\subsection{Dynamics}
\label{sec:coredynamics}
...
Let us now define the operational semantics of core activities, in token passing style.
...
More formally, the operational semantics of core activities $\mathcal{A} = (\mathcal{N},\mathcal{E})$ is defined in terms of labeled reduction rules $\rightarrow~\subseteq \cramped{\Config \times \mathcal{N} \times \Config}$.
The notation $c \xrightarrow{\corenode} c'$ is used as shorthand for $(c, \corenode, c') \in~\rightarrow$.
The two rules are:
\begin{mathpar}
	\inferrule[And] {
		\incoming(\corenode) = \incoming(\corenode, \sigma)
		\\
		\incoming(\corenode) \subseteq \Sigma
		\\
		(\outgoing(\corenode) \setminus \incoming(\corenode)) \cap \Sigma = \emptyset
		\\
		\guard(\alpha)(\sigma)
		\\
		\sigma' \in \effects(\alpha)(\sigma)
		\\
		\outgoing(\corenode) = \outgoing(\corenode, \sigma')
	} {
		(\Sigma,\sigma) \xrightarrow{\corenode} ((\Sigma \setminus \incoming(\corenode)) \cup \outgoing(\corenode),\sigma')
	} {
		\text{ for } \corenode = (\ell, \andnode{\alpha})
	}

	\inferrule[Or] {
		\coreedge \in \incoming(\corenode, \sigma) \cap \Sigma
		\\
		\guard(\alpha)(\sigma)
		\\
		\sigma' \in \effects(\alpha)(\sigma)
		\\
		\coreedge' \in \outgoing(\corenode, \sigma') \setminus (\Sigma \! \setminus \! \{\coreedge\})
	} {
		(\Sigma,\sigma) \xrightarrow{\corenode} ((\Sigma \setminus \{\coreedge\}) \cup \{\coreedge'\}, \sigma')
	} {
		\text{ for } \corenode = (\ell, \ornode{\alpha})
	}
\end{mathpar}

The \textsc{And} rule defines the execution of $\andlabel$-type nodes $\corenode$, for which $\action(\corenode) = \andnode{\alpha}$.
\textsc{And} has six premises.
It requires: (1) the outgoing guards of all incoming edges into $\corenode$ to hold with respect to the current state $\sigma$; (2) all incoming edges into $\corenode$ to have a token; (3) none of the outgoing edges of $\corenode$ except self-loops to have a token, otherwise action execution might attempt to put more than one token on a single edge, which is not allowed; (4) the action guard $\guard(\alpha)$ to hold with respect to $\sigma$; (5) a successor state $\sigma'$ to be available from the effects $\effects(\alpha)$ of $\alpha$; and (6) the incoming guards of all outgoing edges to hold with respect to $\sigma'$.
Note that premises (1) and (2) together mean that all incoming edges into $\corenode$ must be enabled in the current configuration.
If all six premises are met, \textsc{And} removes all tokens from $\incoming(\corenode)$ and puts tokens on $\outgoing(\corenode)$, making $\cramped{(\Sigma \setminus \incoming(\corenode)) \cup \outgoing(\corenode)}$ the new token arrangement.

The \textsc{Or} rule defines the execution of $\orlabel$-type nodes $\corenode$, for which $\action(\corenode) = \ornode{\alpha}$.
\textsc{Or} has four premises.
It requires: (1) the existence of an incoming edge $\coreedge$ of $\corenode$ that is enabled, i.e., that has a token and whose outgoing guard holds; (2) the guard of $\alpha$ to hold with respect to the current state $\sigma$; (3) a successor state $\sigma'$ to be available from $\alpha$'s effects; and (4) the existence of an outgoing edge $\coreedge'$ of $\corenode$ which does not hold a token unless it equals $\coreedge$, and whose incoming guard holds with respect to $\sigma'$.
...
If all four premises are met, \textsc{Or} removes the token from $\coreedge$ and puts it on $\coreedge'$, making $\cramped{(\Sigma \setminus \{\coreedge\}) \cup \{\coreedge'\}}$ the new arrangement of tokens.

Finally, as a shorthand notation, we may write $c \xrightarrow{\alpha} c'$ if there exists an $\corenode \in \mathcal{N}$ such that $c \xrightarrow{\corenode} c'$ and $\action(\corenode) = \alpha$.
...
This section defines concrete activities (\autoref{sec:concrstatics}) that more closely resemble UML activities than core activities, and defines their operational semantics in terms of translations to core activities (\autoref{sec:concrdynamics}).
...
\paragraph{Nodes.}
In contrast to core nodes as defined earlier in \autoref{sec:corestatics}, which come only in two flavours ($\andlabel$ and $\orlabel$), UML activities have many more flavours of activity nodes, like initial nodes, fork nodes, etc.
...
Concrete activities $A = (N, E)$ are graphs with $N \subseteq \ConcrNode$ a set of concrete activity nodes and $E \subseteq N \times \Guard \times \Guard \times N$ a set of edges that have an incoming and outgoing guard.
...
Moreover, note that every edge has an incoming and outgoing guard, rather than, e.g., just the ones coming out of decision nodes.
This is slightly different from UML activities, since in UML, any edge can only have a single guard, and this guard is ignored unless the edge comes out of a decision node.
Any edge guards that are not ignored are then considered to be incoming guards in UML.
Our definition of concrete activities is therefore more general than the standard UML activities, since we also support outgoing guards, in addition to incoming ones.
Outgoing edge guards can be used to impose extra (waiting) conditions in order for some action node to be performed, in addition to the action guard.

\subsection{Dynamics}
\label{sec:concrdynamics}

We now define the operational semantics of concrete activities, by translating concrete activities to core activities, and using the reduction rules defined earlier.
...
We do this by defining a translation function $\denotn{~\cdot~} : \ConcrNode \rightarrow \CoreNode$ that translates concrete nodes to core nodes.
...
This translation function maps concrete nodes to either $\andlabel$-typed or $\orlabel$-typed core nodes, where the type determines the execution semantics.
...
Decision nodes ($\decision$) are translated as $\orlabel$-typed nodes, as their execution requires a single incoming edge (usually just one) to be enabled and nondeterministically chooses a single outgoing edge whose incoming guard holds to receive a token.
...
Merge nodes ($\merge$) are translated similarly: their execution requires a single incoming edge (usually multiple) to be enabled, and puts a token on one outgoing edge (usually just one).
...
Let $\denote{~\cdot~} : \ConcrNode \times \Guard \times \Guard \times \ConcrNode \rightarrow \CoreNode \times \Guard \times \Guard \times \CoreNode$ be the translation function for edges, defined such that
$$\denote{(n,g_{\mathit{in}},g_{\mathit{out}},n')} = (\denotn{n},g_{\mathit{in}},g_{\mathit{out}}, \denotn{n'}).$$
...
Then $(\{\coreedge\}, \sigma_I) \in \Config$ is defined to be an \emph{initial configuration of $A$} for any choice of initial state $\sigma_I \in \State$, if there exists an initial node $(\ell, \init) \in N$ such that $\coreedge \in \outgoing(\denotn{(\ell, \init)},\sigma_I)$.
In other words, any outgoing edge of any initial node can form an initial configuration together with some initial state $\sigma_I$, given that this edge is allowed by its incoming guard to receive an (initial) token in the initial state $\sigma_I$.
...
\begin{itemize}
	...
	\item In fUML, every edge can only have a single guard, which only has actual meaning in case the edge goes out of a decision node.
	In that case, the edge guard is considered to be the incoming guard of that edge.
	Our definition of activities supports two types of edge guards: incoming and outgoing ones.
	Moreover, with our definition, every edge guard has a meaning, as opposed to just the ones that are related to decision nodes.
	\item Our definition of activities imposes a \emph{waiting semantics}, meaning that execution of some action $\alpha$ `waits' until $\guard(\alpha)$ holds, and likewise for incoming and outgoing edge guards.
...
\begin{tabular}{cl}
	...
	$\andnode{\alpha}$ & A core node type describing an $\andlabel$-style execution of $\alpha$ \\
	...
	$\denote{~\cdot~}$ & Operation for translating concrete activity edges to core ones \\
	$\denotn{~\cdot~}$ & Operation for translating concrete activity nodes to core ones \\
	...
	$\coreedge$ & An edge of a core activity \\
	...
	$g_{\mathit{in}}$ & The incoming guard of an edge \\
	$g_{\mathit{out}}$ & The outgoing guard of an edge \\
	...
	$\incomingguard$ & Projection function for obtaining the incoming guard of an edge \\
...
\begin{tabular}{cl}
	...
	$\corenode$ & A node of a core activity \\
	$n$ & A concrete node of a concrete activity \\
	$\ornode{\alpha}$ & A core node type describing an $\orlabel$-style execution of $\alpha$ \\
	...
	$\outgoingguard$ & Projection function for obtaining the outgoing guard of an edge \\
	...
	$\xrightarrow{\corenode}$ & Reduction relation for a core node $\corenode$ \\
