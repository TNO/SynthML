...
\item[] $^\bullet n = \{ n \mid (e, n) \in F \} =$ the set of input edges of $n$
\item[] $n \, ^\bullet = \{ n \mid (e, n) \in F \} =$ the set of output edges of $n$
\end{itemize}
...
We assume that all considered core activities are safe.
...
\ap{notation to be updated with edges instead of transitions}
...
\begin{enumerate}
\item The initial configuration $c_0$ does not place any token on $t_2$
...
\caption{The series edge merging scenario. \ap{Pictures to be improved, and I am not sure why these are so ``decadent''}.}
\label{fig:series-edge-merging}
\end{figure}

\begin{proof}
We must show that $L(\coreActivity) - \{ t_1, t_2 \} \subseteq L(\coreActivity') - \{ t_F \}$ and that $L(\coreActivity) -\{ t_1, t_2 \} \supseteq L(\coreActivity') - \{ t_F \}$. 

Let us first show that $L(\coreActivity) - \{ t_1, t_2 \} \subseteq L(\coreActivity') - \{ t_F \}$; we do so by induction on the number of firings of $t_2$. 
Note that we are interested in \emph{non-blocking} sequences, so we should always be able to take transition $t_2$ and execute its outgoing node (which might be a final node or not). 
First, note that $\coreActivity - \{ t_1, t_2 \} = \coreActivity' - \{ t_F \}$.
For the base case, let $\varsigma$ be a schedule that contains no firing of $t_2$.
If also $t_1 \notin \varsigma$, then $\varsigma \in L(\coreActivity - \{ t_1, t_2, n \})$, and we can use $\varsigma' = \varsigma$ in $\coreActivity'$ to satisfy our claim. 
If instead $t_1 \in \varsigma$, note that there must be exactly one firing of $t_1$ in $\varsigma$ since $\coreActivity$ is safe. 
Let $\varsigma = \varsigma_1 (t_1) \varsigma_2$ where neither $t_1$ or $t_2$ appear in $\varsigma_1$, $\varsigma_2$.  By definition, $g_1$ must be true at the end of $\varsigma_1$ for $t_1$ to take place. 
Note that $t_1$ cannot be enabled at the end of $\varsigma_2$ -- since $t_2$ has not fired yet -- because the activity is safe and node $n$ has no other outgoing edges. 
Then, we can use $\varsigma' = \varsigma_1 (t_F) \varsigma_2$ as a schedule for $\coreActivity'$ -- $t_F$ also can be taken if and only if $g_1$ holds -- and our claim is satisfied. 

Let us now assume that the claim is true until $k$ firings of $t_2$, and consider a schedule $\varsigma$ of $\coreActivity$ where there are $k+1$ firings of $t_2$. 
Since  $t_1 \, ^\bullet = \{ n \} = \ ^\bullet t_2$, activity $\coreActivity$ is safe, and $t_2$ is not in the initial configuration, $t_1$ must fire exactly once before each firing of $t_2$. 
Hence, $\varsigma$ contains either $k+1$ or $k+2$ firings of $t_1$. 
Let consider a schedule with $k+1$ firings of $t_1$: we can write 
$$ \varsigma = \varsigma_1 (t_1) v_1 (t_2) \varsigma_2 (t_1) v_2 \ldots \varsigma_{k+1}(t_1) v_{k+1} (t_2) \varsigma_{k+2}, $$
where neither $t_1$ or $t_2$ appears in $\varsigma_i$ nor $v_i$. 
By inductive hypothesis, we can set $\varsigma' =  \varsigma_1 (t_1) v_1 (t_2) \varsigma_2 (t_1) v_2 \ldots \varsigma_{k+1}$ such that the two languages are equivalent (until step $k$). 
If transition $t_1$ does not share any input nodes, then then final firing in $\varsigma$  must happen when $g_2 \wedge g_2$ holds. 
Let us now analyze $v_{k+1}$: if during this sequence of firings the values of $g_2$, $g_3$, $g_4$, do not change -- and in particular must all be true, since we have a firing of $t_2$ afterwards -- we can simply adapt the final part of the schedule as $\varsigma_{k+1} (t_F) v_{k+1} \varsigma_{k+2}$ or $\varsigma_{k+1} v_{k+1} (t_F) \varsigma_{k+2}$ without loss of generality, and again our claim is satisfied. 
In general however, sequence $v_{k+1}$ can change the truth values of $g_2$ or $g_3$ (Note that at the end of $v_{k+1}$ guard $g_4$ must hold, since we have a firing of $t_2$). 
...
Let us now show that  $L(\coreActivity) - \{ t_1, t_2 \} \supseteq L(\coreActivity') - \{ t_F \}$. 
...
We proceed by induction in the number of steps of $t_F$. 
If there are no firings of $t_F$, the proof follows analogously to the previous case scenario. 
Let us now take the inductive hypothesis, and consider a schedule
$$ \varsigma' = \varsigma'_1 (t_F) \varsigma'_2 (t_F) \ldots \varsigma'_{k+1}(t_F) \varsigma'_{k+2},  $$
that contains $k+1$ firings of $t_F$, and the $ \varsigma_i$ do not contain any firing of $t_F$.
Note that $\varsigma_{k+1}$ enables the firing of $t_F$ by definition: this means that, by inductive hypothesis, there exists a schedule $\varsigma$ in $\coreActivity$ where transition $t_1$ is enabled.
The guards as well simply mirror this, as if $g_2 \wedge g_3 \wedge g_4$ holds, so does $g_2 \wedge g_3$ (the condition to enable $t_1$). 
Thus, transition $t_1$ can freely fire: we extend the schedule $\varsigma$ for $\coreActivity$ by appending the firing of $t_1$ at the end of $\varsigma$. 
To that, we append also $(t_2) \varsigma'_{k+2}$: since $^\bullet t_F = \, ^\bullet t_1$, if the last transition of $\varsigma'_{k+1}$ enables $t_F$, then it also enables $t_1$. 
\end{proof}
...
If there exist two transitions $t_1$ with $\incomingguard(t_1)= g_1$, $\outgoingguard(t_1)= g_2$, and $t_2$ with $\incomingguard(t_2)= g_3$, $\outgoingguard(t_2)= g_4$, and a node $n$ satisfying: 
\begin{enumerate}
\item The initial configuration $c_0$ does not place any token on $t_1$
\item $t_1 \, ^\bullet = \{ n \} = \ ^\bullet t_2$, i.e. node $n$ is the unique output node of $t_1$ and the unique input node of $t_2$
\item Node $n$ is disconnected from all other transitions, and has no effects
\item $(^\bullet t_1)^\bullet = \{ t_1 \}$, i.e. transition $t_1$ does not share any of its input place with any other transitions
\end{enumerate}
then transitions $t_1$ and $t_2$ can be replaced by a single transition $t_F$ with input set $^\bullet t_F = \ ^\bullet t_1$, with output set $ t_F \, ^\bullet = t_2 \, ^ \bullet$ and guards $\incomingguard(t_F)= g_1 \wedge g_2 \wedge g_3$, $\outgoingguard(t_F)=  g_4$.
\end{simplificationrule}

\begin{proof}
The proof *should* follow similarly, perhaps with induction on $t_1$ for the first part. \ap{\#TODO}
\end{proof}
