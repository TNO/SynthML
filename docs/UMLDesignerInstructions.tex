...
A UML class represents a (sub)system that has state and behaviors. An \emph{active} class starts its classifier behavior (\eg an activity) as a direct consequence of its creation.  
...
\footnotetext{A da atype can also be added as an `Packaged Element' by right clicking on the model $\to$ Packaged Element $\to$ Class. The same consideration regarding the interchange of owned types and packaged elements mentioned earlier are valid. Since a data types is usually considered as a type, it is advised to use the owned type option.}
...
\begin{enumerate}
    ...
    \item Create a opaque behavior that models a bit flipper action, with guard \verb#init = true# and effect 
