...
\begin{example*}[Running Example]
The translation of the UML \texttt{BitFlipper} model to CIF is shown in Listing~\ref{alg:bitflipper-cif}. In the CIF model, first the events are defined, either controllable (denoting the start of an action) or uncontrollable (usually describing a nondeterministic or external event).  
...
\begin{example*}[Running Example]
The minimized CIF model is shown in Listing~\ref{alg:bitflipper-minimized-cif}. The CIF model has a \texttt{\_\_start} and an \texttt{\_\_end} event, denoting the start and end of the activity to be synthesized. 
