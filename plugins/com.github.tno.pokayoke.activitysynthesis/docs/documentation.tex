...
\begin{example*}[Running Example]
The translation to CIF of the BitFlipper model is shown in Algorithm~\ref{alg:bitflipper-cif}. The CIF model first defines the events, either controllable (denoting the start of an action) or uncontrollable (usually describing a nondeterministic or external event).  
The BitFlipper model provides two controllable events, \texttt{Flipper} and \texttt{ND\_BitInit} that relate to the opaque behaviors defined in the UML model. 
...
\end{itemize}
This model is the input to Petrify, that translates it into a Petri net. 
\end{example*}
...
\begin{figure}
    ...
    \caption{The BitFlipper activity synthesized.}
